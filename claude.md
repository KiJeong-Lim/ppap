...
1. `Hol BETA2` (진행 중):
   ...
   2. (진행 중) 추가적인 구현 사항:
      ...
      2. (완료됨) 새로운 기능: symbolic calculus. 예: `Y : (C : (|- nat), x : nat |- nat)`일 때 `Y is C * (x + 1) * (x + 2)` => `Y := C * x * x + 3 * C * x + 2 * C`. 기능을 만들었다고 쳤을 때, 다음 쿼리로 확인가능하다:
         ```
      ...
      5. (부분 완료됨: 타입 에러 경로) Z.Doc(Z.Text.Doc 아님)을 이용하여 에러 메세지 깔끔하게 출력하기. (칼라풀하게도 할 수 있으면 더 좋아. `src/Z/PC.hs`의 L307-L316 참고할 것.)
      6. (부분 완료됨: 타입 에러 메시지 포맷) 아무래도 타입 에러 메세지가 너무 구려. GHC식 타입체킹 알고리즘 및 에러 메세지 적용하는 게 가능할까?
   3. (대기 중) 확인할 점:
