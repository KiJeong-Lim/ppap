...
1. `Hol BETA2` (진행 중):
   ...
   2. (진행 중) 추가적인 구현 사항:
      ...
      5. (대기 중) Z.Doc(Z.Text.Doc 아님)을 이용하여 에러 메세지 깔끔하게 출력하기. (칼라풀하게도 할 수 있으면 더 좋아. `src/Z/PC.hs`의 L307-L316 참고할 것.)
      6. (대기 중) 아무래도 타입 에러 메세지가 너무 구려. GHC식 타입체킹 알고리즘 및 에러 메세지 적용하는 게 가능할까?
   3. (대기 중) 확인할 점:
