...
genLexer xblocks = do
    ...
    fmap (strcat . snd) $ runWriterT $ do
        ...
        tellLine (strstr "    runDFA (DFA q0 qfs deltas markeds pseudo_finals) = XIdentity.runIdentity . go where")
        ...
        tellLine (strstr "                Just p ")
        tellLine (strstr "                    | p `XSet.member` pseudo_finals -> return (buffer ++ [ch] ++ str)")
        tellLine (strstr "                    | otherwise -> case XMap.lookup p qfs of")
