...
(1) RULE[ FlexFlex(same heads) ]
    ...
    > env'' := env'.zonk(X +-> lam(n + l). app(H, zs)).
    ============================================================================================================
...
(2) RULE[ binding ]
    ...
    > env.evalref X == none.
    > env ~~> Bind_{0}[ app(X, [a_1 .. a_n]) +-> t ] = s ~~> env', if probs satisfied.
    > env'' := env'.update(X +-> lam(n). s).
    ==================================================================================
...
{- ${Env} ~~> Bind_{${Nat}}[ ${Term} +-> ${Term} ] = ${Term} ~~> ${Env}, if ${Constraints} satisfied.
(1) RULE[ BindLam ]
    > env ~~> Bind_{l + m}[ app(X, [a_1 .. a_n]) +-> t ] = s ~~> env', if probs satisfied.
    > m is a positive integer.
    ==================================================================================================
    > env ~~> Bind_{l}[ app(X, [a_1 .. a_n]) +-> lam(m). t ] = lam(m). s ~~> env', if probs satisfied.
(2) RULE[ FlexRigid ]
    ...
    > For k in [1 .. m],
    > env_{k - 1} ~~> Bind_{l}[ app(X, [a_1 .. a_n]) +-> t_k ] = s_k ~~> env_{k}, if probs_k satisfied.
    ======================================================================================================================================================
    > env_{0} ~~> Bind_{l}[ app(X, [a_1 .. a_n]) +-> app(r, [t_1 .. t_m]) ] = app(r', [s_1 .. s_m]) ~~> env_{m}, if concat [probs_1 .. probs_m] satisfied.
(3) RULE[ FlexFlex(different heads, rhs head invisible) ]
    ...
    > env' = env
    >   |> new flex var H = { evalref = none, scope = env.scope X }.
    > env'' := env'.update(Y +-> app(H, cs ++ us)).
    ==================================================================================================================
    > env ~~> Bind_{l}[ app(X, [a_1 .. a_n]) +-> app(Y, [b_1 .. b_m]) ] = app(H, ws ++ vs) ~~> env'', if [] satisfied.
(4) RULE[ FlexFlex(different heads, rhs head visible) ]
    ...
    > env' = env
    >   |> new flex var H = { evalref = none, scope = env.scope Y }.
#                                                     ^^^^^^^^^^^ In [1], it is `env.scope X`.
    > env'' := env'.update(Y +-> app(H, ws ++ vs)).
    ==================================================================================================================
    > env ~~> Bind_{l}[ app(X, [a_1 .. a_n]) +-> app(Y, [b_1 .. b_m]) ] = app(H, cs ++ us) ~~> env'', if [] satisfied.
-}
