...
runTimedProcess timeoutMicros cwd envOverrides command args input = do
    ...
    return
        TimedProcessResult
            { tprCommand = unwords (command : args)
            , tprExitCode = exitCode
            , tprTimedOut = maybe True (const False) exitCode
            , tprStdout = out
            , tprStderr = err
            }
...
processLog result =
    ProcessLog
        { plCommand = tprCommand result
        , plExitCode = fmap exitCodeNumber (tprExitCode result)
        , plTimedOut = tprTimedOut result
        , plStdout = tprStdout result
        , plStderr = tprStderr result
        }
...
obsFromProcess result =
    Obs
        { obsTermination = termination
        , obsExitCode = fmap exitCodeNumber (tprExitCode result)
        , obsStdout = tprStdout result
        , obsTimedOut = tprTimedOut result
        }
  where
    termination
...
makeProcessEnv overrides = do
    ...
    let keys = map fst overrides
        inherited = filter (\(key, _) -> key `notElem` keys) base
