...
genProgram seed size
    ...
    | otherwise = Program ([SVar TInt "x" (EInt x0), SVar TInt "y" (EInt y0)] ++ guardedIf ++ guardedLoop ++ [SPrintln [EAdd (EVar TInt "x") (EVar TInt "y")]])
    where
        n :: Int
        n = abs seed
        ...
        guardedIf :: [Stmt]
        guardedIf = if size >= 2 then [SIf (ELt (EVar TInt "x") (EVar TInt "y")) [SAssign "x" (EAdd (EVar TInt "x") (EInt 1))] [SAssign "y" (EAdd (EVar TInt "y") (EInt 1))]] else []

        guardedLoop :: [Stmt]
        guardedLoop = if size >= 3 then [SForBounded "i" (1 + (n `mod` 4)) [SAssign "x" (EAdd (EVar TInt "x") (EVar TInt "i"))]] else []
