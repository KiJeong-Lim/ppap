...
import Hol.BETA2.TypeChecker (inferTypeWithDiagnostic)
...
makeKindEnv mode sourceLines = go where
    ...
    unRep krep = do
        (kin, loc) <- case krep of
            ...
            RKPrn loc krep -> do
                ...
                return (kin, loc)
        if getRank kin > 1
            then Left (desugarErr mode sourceLines loc "The higher-order kind expression is not allowed.")
            else return kin
    ...
    go ((loc, (tcon, krep)) : triples) kind_env
        | TC_Named (tc : _) <- tcon, tc `elem` ['A' .. 'Z'] = Left (desugarErr mode sourceLines loc "The identifier of a type constructor must be started with a small letter.")
        | otherwise = case Map.lookup tcon kind_env of
            Just _ -> Left (desugarErr mode sourceLines loc "It is wrong to redeclare an already declared type constructor.")
            Nothing -> do
                kin <- unRep krep
...
typeRepToMono mode sourceLines kind_env = go where
    ...
    go trep = case trep of
        ...
        RTyCon loc type_constructor -> case Map.lookup type_constructor kind_env of
            Nothing -> Left (desugarErr mode sourceLines loc ("The type constructor `" ++ showsPrec 0 type_constructor "' has not been declared."))
            Just kin -> return (kin, TyCon (TCon type_constructor kin))
        RTyApp loc trep1 trep2 -> do
            (kin1, typ1) <- go trep1
            (kin2, typ2) <- go trep2
            case applyModusPonens kin1 kin2 of
                Left msg -> Left (desugarErr mode sourceLines loc (dropWhile (== ' ') msg ++ "."))
...
makeTypeEnv mode sourceLines kind_env = go where
    unRep = typeRepToMono mode sourceLines kind_env
            ...
            Nothing -> do
                ...
                if kin == Star then
                    if hasValidHead typ then
                        ...
                        Left (desugarErr mode sourceLines loc ("The head of the type `" ++ showsPrec 0 con "' is invalid."))
                else
                    Left (desugarErr mode sourceLines loc ("Couldn't solve `" ++ pprint 0 kin "' ~ `type'."))
            _ -> Left (desugarErr mode sourceLines loc ("It is wrong to redeclare the already declared constant `" ++ showsPrec 0 con "'."))
...
desugarProgramWithDiagnostic mode sourceLines kind_env type_env file_name program0
    = let program = program0
      ...
      in case makeKindEnv mode sourceLines [ (loc, (tcon, krep)) | RKindDecl loc tcon krep <- program ] kind_env of
            Left err_msg -> throwE err_msg
            Right kind_env' -> case makeTypeEnv mode sourceLines kind_env' expandedTypes type_env of
                Left err_msg -> throwE err_msg
                Right type_env' -> case populateTypeFoldTable mode sourceLines kind_env' expansion_db notation_db0 of
                    Left err_msg -> throwE err_msg
                    Right notation_db1 -> do
                        notation_db <- populateTermFoldTable mode sourceLines type_env' expansion_db notation_db1
...
populateTypeFoldTable mode sourceLines kind_env expansion_db = go (Notation.typeAbbrevList expansion_db)
  where
    go [] db = Right db
    go ((name, params, rhs) : rest) db = do
        let expanded = Notation.expandTypeRep expansion_db rhs
        (_, monoType) <- typeRepToMono mode sourceLines kind_env expanded
...
populateTermFoldTable mode sourceLines type_env expansion_db db0 = go (Notation.termNotationList expansion_db) db0
  where
    go [] db = return db
    go ((name, params, rhs) : rest) db = do
        let expanded = Notation.expandTermRep expansion_db rhs
        mTemplate <- lift (runExceptT (compileNotationRHS mode sourceLines db0 type_env params expanded))
        case mTemplate of
            Left _    -> go rest db
...
compileNotationRHS mode sourceLines db type_env params body = do
    ...
    ((typedExpr, assumptions), used_mtvs) <- inferTypeWithDiagnostic mode sourceLines db type_env typedTerm
