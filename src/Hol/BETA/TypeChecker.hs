...
getMGU lhs rhs
    ...
    where
        ...
        go lockeds (TyVar _) _
            = error "`getMGU\'"
        go lockeds _ (TyVar _)
            = error "`getMGU\'"
...
inferTypeWithModule mode moduleName source_lines db type_env = flip runStateT Map.empty . infer where
    ...
    mkUnknownConErr mode' moduleName' source loc con =
        diagnosticWithModule mode' "HolBETA-NotInScope" moduleName' source loc [Z.Doc.text ("Not in scope: data constructor `" ++ showsPrec 0 con "'") , Z.Doc.text "Add a `type' declaration for it, or check the spelling."]
